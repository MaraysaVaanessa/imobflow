...
const TenantLogin = () => {
  ...
  const handleLogin = async () => {
    ...
    setCarregando(true);
    try {
      const response = await fetch(`${API_URL}/tenant-portal/login`, {
        method: "POST",
      ...
      if (data.precisaTrocarSenha) {
        navigate("/portal-inquilino/trocar-senha");
      } else {
        navigate("/portal-inquilino/dashboard");
      }
    } catch (err) {
    ...
    <div className="flex min-h-screen w-full items-center justify-center bg-lightPrimary dark:bg-navy-900">
      <div className="w-full max-w-[420px] rounded-2xl bg-white p-8 shadow-xl dark:bg-navy-800">
        ...
        <p className="mb-6 text-sm text-gray-600 dark:text-gray-300">
          Acesse para ver seu contrato, pagamentos e abrir chamados de
          manutenção.
        </p>
